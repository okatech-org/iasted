    ...
    const connect = useCallback(async (voice: 'echo' | 'ash' | 'shimmer' = 'echo', systemPrompt?: string) => {
        ...
        try {
            ...
            dcRef.current.addEventListener("open", () => {
                ...
                // Instructions système : Priorité absolue au systemPrompt passé en paramètre
                // S'il est absent, on utilise le fallback
                // Construire les instructions finales
                const finalInstructions = `
                    ${systemPrompt || 'Tu es un assistant utile.'}
                    
                    IMPORTANT: Tu dois TOUJOURS parler en FRANÇAIS, sauf si l'utilisateur te demande explicitement une autre langue.
                    Ta voix doit être fluide, naturelle et professionnelle.
                    Ne mentionne jamais que tu es une IA ou un modèle de langage, agis comme un assistant humain compétent.
                `.trim();

                console.log('📝 [WebRTC] Instructions envoyées (Length):', finalInstructions.length);
                    ...
                    session: {
                        ...
                        tools: [
                            ...
                            },
                            {
                                type: 'function',
                                name: 'create_project',
                                description: 'Ouvre la fenêtre de création d\'un nouveau projet ou hub.',
                                parameters: {
                                    type: 'object',
                                    properties: {
                                        name: { type: 'string', description: 'Nom du projet (optionnel)' }
                                    },
                                    required: []
                                }
                            },
                            {
                                ...
                                description: 'Lance l\'implémentation ou le développement d\'un projet existant.',
                                parameters: {
                                    type: 'object',
                                    properties: {
                                        project_name: { type: 'string', description: 'Nom du projet à implémenter' }
                                    },
                            ...
                            },
                            {
                                ...
                                description: 'Analyse le projet actuel (fichiers, structure, contexte). Utilise cet outil quand l\'utilisateur demande d\'analyser le code, le projet, ou de "rentrer dans l\'ordi".',
                                parameters: {
                                    type: 'object',
                                    properties: {
                                        scope: {
                                            type: 'string',
                                            enum: ['full', 'structure', 'dependencies'],
                                            description: 'Portée de l\'analyse'
                                        },
                                        path: {
                                            type: 'string',
                                            description: 'Chemin spécifique à analyser (optionnel)'
                                        }
                                    },
                                    required: ['scope']
                                }
